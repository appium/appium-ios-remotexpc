...
interface ChainableAssertion {
  ...
  length: ((expected: number, message?: string) => ChainableAssertion) &
    ChainableAssertion;
  ...
  rejectedWith: (
    ...expected: Array<
      RegExp | string | (abstract new (...args: never[]) => Error)
    >
  ) => Promise<ChainableAssertion>;
...
const wrapHook =
  (fn: HookFn): any =>
  async (t: TestContext) =>
    fn.call(makeContext(t));
const wrapSuite =
  (fn: SuiteFn): any =>
  async (t: TestContext) =>
    fn.call(makeContext(t));
const wrapTest = (fn: TestFn) => async (t: TestContext) =>
  fn.call(makeContext(t), t);
...
const assertMembers = (actual: unknown, expected: unknown[]) => {
  ...
  for (const member of expected) {
    assert.ok(
      actual.includes(member),
      `expected array to include ${String(member)}`,
    );
  }
};

const makeIncludeAssertion = (state: AssertionState) => {
  const includeAssertion = ((expected: unknown) =>
    compare(state, () => includes(state.actual, expected))) as ((
    expected: unknown,
  ) => ChainableAssertion) &
    ChainableAssertion;
    ...
    get(target, property) {
      if (property === 'members') {
        return (expected: unknown[]) =>
          compare(state, () => assertMembers(state.actual, expected));
      }
...
const makeLengthAssertion = (state: AssertionState) => {
  ...
  const lengthAssertion = ((expected: number) =>
    compare(lengthState, () =>
      assert.equal(lengthState.actual, expected, state.message),
    )) as ((expected: number) => ChainableAssertion) & ChainableAssertion;
      ...
      switch (property) {
        ...
        case 'an':
          return (typeName: string) =>
            compare(state, () =>
              assertType(state.actual, typeName, state.message),
            );
        case 'closeTo':
          return (expected: number, delta: number, message?: string) =>
            compare(state, () =>
              assert.ok(
                Math.abs((state.actual as number) - expected) <= delta,
        ...
        case 'empty':
          return compare(state, () =>
            assert.equal(getLength(state.actual), 0, state.message),
          );
        case 'equal':
        ...
        case 'exist':
          return compare(state, () =>
            assert.notEqual(state.actual, null, state.message),
          );
        case 'false':
          return compare(state, () =>
            assert.equal(state.actual, false, state.message),
          );
        case 'fulfilled':
        ...
        case 'gt':
          return (expected: number | bigint, message?: string) =>
            compare(state, () =>
              assert.ok(
                (state.actual as number | bigint) > expected,
                message ?? state.message,
              ),
            );
        ...
        case 'instanceof':
          ...
          return (expected: abstract new (...args: never[]) => unknown) =>
            compare(state, () =>
              assert.ok(state.actual instanceof expected, state.message),
            );
        case 'keys':
          return (...expected: string[]) =>
            compare(state, () =>
              assert.deepEqual(
                Object.keys(Object(state.actual)).sort(),
                expected.sort(),
              ),
            );
        case 'least':
        ...
        case 'match':
          return (expected: RegExp) =>
            compare(state, () => assert.match(String(state.actual), expected));
        case 'members':
          return (expected: unknown[]) =>
            compare(state, () => assertMembers(state.actual, expected));
        case 'most':
        ...
        case 'null':
          return compare(state, () =>
            assert.equal(state.actual, null, state.message),
          );
        case 'oneOf':
          return (expected: unknown[]) =>
            compare(state, () => assert.ok(expected.includes(state.actual)));
        case 'property':
        ...
        case 'satisfy':
          return (predicate: (actual: any) => boolean) =>
            compare(state, () =>
              assert.ok(predicate(state.actual), state.message),
            );
        case 'throw':
        case 'throws':
          return (
        ...
        case 'true':
          return compare(state, () =>
            assert.equal(state.actual, true, state.message),
          );
        case 'undefined':
          return compare(state, () =>
            assert.equal(state.actual, undefined, state.message),
          );
        default:
          return undefined;
      }
...
globalThis.expect = Object.assign(
  (actual: unknown, message?: string) =>
    createAssertion({ actual, deep: false, message, negate: false }),
  {
    fail(message?: string): never {
      assert.fail(message);
    },
