...
describe('MobileImageMounterService Integration', function () {
  ...
  describe('Image Lookup Operations', () => {
    ...
    it('should copy devices list (test CopyDevices command)', async function () {
      try {
        const devices = await serviceWithConnection!.mobileImageMounterService.copyDevices();
  ...
  describe('Personalization Support', () => {
    it('should query personalization identifiers only', async function () {
      try {
        const identifiers = await serviceWithConnection!.mobileImageMounterService.queryPersonalizationIdentifiers();
        expect(identifiers).to.be.an('object');
        
        console.log(JSON.stringify(identifiers, null, 2));

      } catch (error) {
        console.log('Personalization identifiers query failed:', (error as Error).message);
        throw error;
      }
    });
    ...
    it('should test queryPersonalizationManifest behavior', async function () {
      ...
      try {
        ...
        const mountedSignatures = await serviceWithConnection!.mobileImageMounterService.lookup();
        ...
        if (mountedSignatures.length > 0) {
          for (let i = 0; i < mountedSignatures.length; i++) {
            ...
            try {
              ...
              const manifest = await serviceWithConnection!.mobileImageMounterService.queryPersonalizationManifest(
                'DeveloperDiskImage',
                sig
              );
              ...
              console.log(`Query with mounted signature ${i + 1} failed:`, errorMessage);
            }
          }
        }
        ...
        const imageFilePath = '/Users/navinchandra/.pymobiledevice3/Xcode_iOS_DDI_Personalized/Image.dmg';
        ...
        const errorMessage = (error as Error).message;
        console.log('QueryPersonalizationManifest failed with error:', errorMessage);
        
        if (errorMessage.includes('MissingManifestError')) {
          console.log('✓ Got expected MissingManifestError - manifest not found on device');
        } else if (errorMessage.includes('Timed out')) {
          console.log('✓ Got timeout error - this means device closed connection');
        } else {
