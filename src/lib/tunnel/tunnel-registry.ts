...
export class TunnelRegistryManager {
  ...
  constructor(registryPath?: string) {
    this.registryPath = registryPath ?? join(process.cwd(), 'tunnel-registry.json');
  }
  ...
  async loadRegistry(): Promise<TunnelRegistry> {
    try {
      ...
      log.warn(`Failed to load tunnel registry from ${this.registryPath}: ${error}`);
      return {
        tunnels: {},
        metadata: {
 ...
 */
export async function getTunnelByUdid(udid: string): Promise<TunnelRegistryEntry | null> {
