...
import type {AppleTVDevice, AppleTVPairingFlowResult, AppleTVPairingResult, PairingConfig} from '../types.js';
...
export class AppleTVPairingService {
  ...
  constructor(userInput: UserInputInterface, config: PairingConfig = DEFAULT_PAIRING_CONFIG) {
    this.config = config;
  ...
  async discoverAndPair(deviceSelector?: string, options: AppleTVPairingOptions = {}): Promise<AppleTVPairingResult> {
    try {
      const devices =
        options.devices ??
  ...
  async pairWithDevice(device: AppleTVDevice): Promise<AppleTVPairingFlowResult> {
    try {
  ...
  async discoverDevices(options: AppleTVPairingDiscoveryOptions = {}): Promise<AppleTVDevice[]> {
    try {
  ...
  private async selectDevice(devices: AppleTVDevice[], deviceSelector?: string): Promise<AppleTVDevice> {
