// Default configuration for Remote Pairing (Wi‑Fi `_remotepairing._tcp`)
// Env: REMOTE_PAIRING_* (preferred) or legacy APPLETV_*
export const DEFAULT_PAIRING_CONFIG = {
  timeout:
    Number(process.env.REMOTE_PAIRING_TIMEOUT) ||
    Number(process.env.APPLETV_TIMEOUT) ||
    30000,
  discoveryTimeout:
    Number(process.env.REMOTE_PAIRING_DISCOVERY_TIMEOUT) ||
    Number(process.env.APPLETV_DISCOVERY_TIMEOUT) ||
    10000,
  maxRetries:
    Number(process.env.REMOTE_PAIRING_MAX_RETRIES) ||
    Number(process.env.APPLETV_MAX_RETRIES) ||
    3,
} as const;
