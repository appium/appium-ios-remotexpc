 ...
 */
export async function withZipFile<T>(ipaPath: string, fn: (zip: ZipArchive) => Promise<T>): Promise<T> {
...
export async function openZipEntryStream(zip: ZipArchive, entry: IpaZipEntry): Promise<Readable> {
