...
export class CrashReportsService extends BaseService {
  ...
  async flush(): Promise<void> {
    ...
    const service = {
      serviceName: CrashReportsService.RSD_CRASH_MOVER_NAME,
      port: this.crashMoverAddress[1].toString(),
    };
    ...
    try {
      ...
      const ack = await new Promise<Buffer>((resolve, reject) => {
        ...
        const onData = (chunk: Buffer) => {
          chunks.push(chunk);
          if (Buffer.concat(chunks).length >= expectedAck.length) {
            socket.removeListener('data', onData);
            resolve(Buffer.concat(chunks).subarray(0, expectedAck.length));
          }
        };
        socket.on('data', onData);
        socket.setTimeout(5000, () => {
          socket.removeListener('data', onData);
          reject(new Error('Timeout waiting for flush acknowledgment'));
        });
      });

      if (!ack.equals(expectedAck)) {
        log.warn(`Unexpected flush acknowledgment: ${ack.toString('hex')}`);
      }
  ...
  private async _pullRecursive(
    ...
    const isDir = await this.afc.isdir(remotePath);
    ...
    if (!isDir) {
      ...
      try {
        ...
        log.debug(`Pulled: ${remotePath} -> ${localPath}`);
        if (erase) {
          await this.afc.rmSingle(remotePath, true);
        }
      } catch (error) {
        log.debug(`Failed to pull file '${remotePath}': ${error}`);
      }
    } else {
      ...
      try {
        ...
        log.debug(`Failed to list directory '${remotePath}': ${error}`);
      }
    }
  }
}
